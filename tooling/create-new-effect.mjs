...
const _TYPE_INFO = {
	"effect": {
		folderJson: JSON.parse(fs.readFileSync(path.join(DIR_PACKS_SOURCE, PACK_ID, "Effects_Qf6H7pepYZi6Oa6S.json"), "utf-8")),
	},
	"manual": {
		folderJson: JSON.parse(fs.readFileSync(path.join(DIR_PACKS_SOURCE, PACK_ID, "Effects__Manual__o8nGPmux6bWEpn9M.json"), "utf-8")),
	},
	"utility": {
		folderJson: JSON.parse(fs.readFileSync(path.join(DIR_PACKS_SOURCE, PACK_ID, "Utilities_pwwwYY4IGpKs8tc1.json"), "utf-8")),
	},
}
...
program
	...
	.addOption(
		new Option("--type <type>")
			.choices(Object.keys(_TYPE_INFO))
			.default("effect"),
	)
	.action((name, {id, img, type}) => {
		...
		const {folderJson: {_id: folderId, name: dirName}} = _TYPE_INFO[type]
