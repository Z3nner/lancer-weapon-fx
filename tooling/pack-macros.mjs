...
if (!fs.existsSync(DIR_PACKS_SOURCE))
    throw new Error(`Unpacked dir "${DIR_PACKS_SOURCE}" does not exists! Run "npm run db:unpack" first`);

// Discover pack source dirs
const packSourceDirs = fs
    .readdirSync(DIR_PACKS_SOURCE)
    .map(name => ({
        name,
