...
const _UNWANTED_DOC_KEYS = ["author", "ownership", "flags", "_stats"];

// Discover pack dirs
const packDirs = fs
    .readdirSync(DIR_PACKS)
    .map(name => ({
        name,
...
for (const { name, dirpath } of packDirs) {
    ...
    await extractPack(dirpath, dirpathOut, {
        clean: true,
        log: true,
    ...
    fs.readdirSync(dirpathOut)
        ...
        .forEach(name => {
            ...
            json.command_source = fpathFolderized
                .replace(/\.json$/, ".js")
                .split(path.sep)
                .join("/");
            fs.writeFileSync(fpathFolderized, JSON.stringify(json, null, 4) + "\n", "utf-8");
