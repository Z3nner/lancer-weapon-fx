...
test("that the algorithm runs with 0 points", () => {
    expect(LloydsAlgorithm.getCentroids([], 2)).toStrictEqual([]);
});

test("that the algorithm runs with points less than the number of clusters", () => {
    expect(LloydsAlgorithm.getCentroids([{ x: 1, y: 1 }], 2)).toStrictEqual([{ x: 1, y: 1 }]);
});

test("that the algorithm runs with duplicate points", () => {
    expect(
...
test("that the algorithm runs with the same number of points as clusters", () => {
    expect(
        LloydsAlgorithm.getCentroids(
            [
                { x: 0, y: 0 },
