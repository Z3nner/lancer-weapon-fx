...
const _getFlowInfo = (state, {fallbackActionIdentifier = null} = {}) => {
	const zippedTargetInfo = Array.from(
		{
			length: Math.max(
				state.data.acc_diff?.targets?.length || 0,
				state.data.hit_results?.length || 0,
			),
		},
		(_, i) => ({
...
const _onReady = () => {
	...
	_bindFlowHook({
		...
		fallbackActionIdentifier: flow => flow.state.data.attack_type === "Melee"
			? "lwfx_default_melee"
			: "lwfx_default_ranged",
	});
